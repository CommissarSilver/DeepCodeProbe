...
parser.add_argument(
    "--data",
    type=str,
    default="/scratch/f/foutsekh/vamaj/CSCGDual/data/python/original",
...
def evaluate(data_source):
    ...
    for i in tqdm(
        range(0, data_source.size(0) - 1, args.bptt), total=len(data_source) // args.bptt
    ):
        data, targets = get_batch(data_source, i, evaluation=True)
        output, hidden = model(data, hidden)
        output_flat = output.view(-1, ntokens)
        total_loss += len(data) * criterion(output_flat, targets).item()  # Using .item()
        hidden = repackage_hidden(hidden)

    return total_loss / len(data_source)  # No need to index total_loss
