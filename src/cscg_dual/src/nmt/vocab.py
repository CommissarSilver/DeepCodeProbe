...
class VocabEntry(object):
    ...
    def from_corpus(corpus, size, remove_singleton=True):
        ...
        non_singletons = [w for w in word_freq if word_freq[w] > 4]
        print(
        ...
        for word in top_k_words:
            if len(vocab_entry) < size:
                if not (word_freq[word] < 4 and remove_singleton):
...
class Vocab(object):
    def __init__(
        self, src_sents, tgt_sents, src_vocab_size, tgt_vocab_size, remove_singleton=True
    ):
        assert len(src_sents) == len(tgt_sents), "{} {}".format(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--src_vocab_size", default=50000, type=int, help="source vocabulary size"
    )
    parser.add_argument(
        "--tgt_vocab_size", default=50000, type=int, help="target vocabulary size"
    )
    parser.add_argument(
        "--include_singleton",
        action="store_true",
    ...
    parser.add_argument(
        ...
        default="/store/travail/vamaj/CSCGDual/data/python/original/train.token.code",
    )
    parser.add_argument(
        "--train_tgt",
        type=str,
        ...
        default="/store/travail/vamaj/CSCGDual/data/python/original/train.token.nl",
    )

    parser.add_argument(
        "--output", default="vocab.bin.pkl", type=str, help="output vocabulary file"
    )
    ...
    pickle.dump(vocab, open(args.output, "wb"))
