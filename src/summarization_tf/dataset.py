...
def make_dict():
    labels = Parallel(n_jobs=-1)(
        delayed(get_labels)(p)
        for p in tqdm(glob("./dataset/tree_raw/train/*"), "reading all labels")
    )
    labels = [l for s in labels for l in s]

    non_terminals = set(
        [get_bracket(x) for x in tqdm(list(set(labels)), "collect non-tarminals")]
    ) - set([None, "(SimpleName)"])
    ...
    ids = Counter(
        [
            y
            for y in [get_identifier(x) for x in tqdm(labels, "collect identifiers")]
            if y is not None
        ]
    )
    ...
    values = Counter(
        [
            y
            for y in [get_values(x) for x in tqdm(labels, "collect values")]
            if y is not None
        ]
    )
...
def preprocess_trees():
    ...
    for sets in sets_name:
        ...
        Parallel(n_jobs=-1)(
            delayed(rebuild_tree)(p, d, dic)
            for p, d in tqdm(list(zip(files, dst)), "preprocessing {}".format(sets))
        )
