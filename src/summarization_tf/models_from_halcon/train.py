...
trn_data = read_pickle("/scratch/f/foutsekh/vamaj/sum_tf/dataset/nl/train.pkl")
vld_data = read_pickle("/scratch/f/foutsekh/vamaj/sum_tf/dataset/nl/valid.pkl")
tst_data = read_pickle("/scratch/f/foutsekh/vamaj/sum_tf/dataset/nl/test.pkl")
code_i2w = read_pickle("/scratch/f/foutsekh/vamaj/sum_tf/dataset/code_i2w.pkl")
code_w2i = read_pickle("/scratch/f/foutsekh/vamaj/sum_tf/dataset/code_w2i.pkl")
nl_i2w = read_pickle("/scratch/f/foutsekh/vamaj/sum_tf/dataset/nl_i2w.pkl")
nl_w2i = read_pickle("/scratch/f/foutsekh/vamaj/sum_tf/dataset/nl_w2i.pkl")
...
# load the model
model.load_state_dict(torch.load("/scratch/f/foutsekh/vamaj/sum_tf/checkpoints/epoch_8.pth"))
writer = SummaryWriter()
for epoch in range(9, epochs + 1):
    batch_turn = 0
    # train
    loss_tmp = []
    t = tqdm(trn_gen(0))
    for x, y, _, _ in t:
        try:
            ...
            batch_turn += 1
            if batch_turn % 100 == 0:
                torch.save(model.state_dict(), f'/scratch/f/foutsekh/vamaj/sum_tf/checkpoints/epoch_{epoch}_batch_{batch_turn}.pth')
        except Exception as e:
            print(e)
            pass
    
    torch.save(model.state_dict(), f'/scratch/f/foutsekh/vamaj/sum_tf/checkpoints/epoch_{epoch}.pth')
