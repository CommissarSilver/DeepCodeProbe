...
class Type4Py(nn.Module):
    ...
    def encode(self, x_id, x_tok, x_type):
        # Using dropout on input sequences
        x_id = self.dropout(x_id)
        x_tok = self.dropout(x_tok)
...
class Type4PyWOI(nn.Module):
    ...
    def __init__(
        ...
        self.linear = nn.Linear(
            self.hidden_size * 2 + self.aval_type_size, self.output_size
        )
...
def create_knn_index(
    train_types_embed: np.array, valid_types_embed: np.array, type_embed_dim: int
) -> AnnoyIndex:
    """
    ...
    for i, v in enumerate(
        tqdm(train_types_embed, total=len(train_types_embed), desc="KNN index")
    ):
...
def train_loop_dsl(
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch_i, (anchor, positive_ex, negative_ex) in enumerate(
            tqdm(train_data_loader, total=len(train_data_loader), desc=f"Epoch {epoch}")
        ):
...
def train(
    ...
    common_types = [
        t.item()
        for t in train_data_loader.dataset.labels
    ...
    logger.info(
        "Percentage of ubiquitous types: %.2f%%"
        % (
            len(
                [
                    t.item()
                    for t in train_data_loader.dataset.labels
                    if t.item() in ubiquitous_types
                ]
            )
            / train_data_loader.dataset.labels.shape[0]
            * 100.0
        )
    )
    logger.info(
        "Percentage of common types: %.2f%%"
        % (
            len(
                [
                    t.item()
                    for t in train_data_loader.dataset.labels
                    if t.item() in common_types
                ]
            )
            / train_data_loader.dataset.labels.shape[0]
            * 100.0
    ...
    with open(
        join(output_path, f"{data_loading_funcs['name']}_common_types.pkl"), "wb"
    ) as f:
