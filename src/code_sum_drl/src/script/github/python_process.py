                ...
                with open(
                    ...
                    for i in range(len(declbodies_lines)):
                        ...
                        if (
                            ...
                            and len(comment.split()) > 2
                            and comment[0].isalpha()
                        ):
...
def split_dataset(train_portion, dev_portion):
    ...
    with open(processed_path + "all.code", "r") as all_file:
        with open(
            train_path + "train%s%s%s.code" % (train_portion, dev_portion, test_portion),
            "w",
        ) as train_file:
            with open(
                train_path
                + "dev%s%s%s.code" % (train_portion, dev_portion, test_portion),
                "w",
            ) as dev_file:
                with open(
                    train_path
                    + "test%s%s%s.code" % (train_portion, dev_portion, test_portion),
                    "w",
                ) as test_file:
    ...
    with open(processed_path + "all.comment", "r") as all_file:
        with open(
            train_path
            + "train%s%s%s.comment" % (train_portion, dev_portion, test_portion),
            "w",
        ) as train_file:
            with open(
                train_path
                + "dev%s%s%s.comment" % (train_portion, dev_portion, test_portion),
                "w",
            ) as dev_file:
                with open(
                    train_path
                    + "test%s%s%s.comment" % (train_portion, dev_portion, test_portion),
                    "w",
                ) as test_file:
                    for a_line in all_file.readlines():
                        if idx in train:
