...
def get_opt():
    ...
    parser.add_argument(
        "-src_seq_length", type=int, default=100, help="Maximum source sequence length"
    )
    parser.add_argument(
        "-tgt_seq_length",
        type=int,
...
def makeData(which, srcFile, tgtFile, srcDicts, tgtDicts):
    ...
    while True:
        ...
        if opt.data_name == "github-python":
            srcLine = python_tokenize(
                sline.replace(" DCNL DCSP ", "")
                .replace(" DCNL ", "")
                .replace(" DCSP ", "")
            )
            tgtLine = (
                tline.replace(" DCNL DCSP ", "")
                .replace(" DCNL ", "")
                .replace(" DCSP ", "")
                .split()
            )
        ...
        if (
            len(srcLine) <= opt.src_seq_length and len(tgtLine) <= opt.tgt_seq_length
...
def main():
    ...
    (
        save_data["train_xe"],
        train_xe_code_sentences,
        train_xe_comment_sentences,
    ) = makeDataGeneral("train_xe", opt.train_xe_src, opt.train_xe_tgt, dicts)
    (
        save_data["train_pg"],
