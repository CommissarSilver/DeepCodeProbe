    ...
    def _convert_and_report(self, data, pred_file, preds, targets, srcs, metrics):
        with open(pred_file, "w") as f:
            for i in range(len(preds)):
                ...
                pred = lib.Reward.clean_up_sentence(
                    pred, remove_unk=False, remove_eos=True
                )
                target = lib.Reward.clean_up_sentence(
                    target, remove_unk=False, remove_eos=True
                )
                ...
                f.write(
                    str(i) + ": src: " + " ".join(src).encode("utf-8", "ignore") + "\n"
                )
                f.write(
                    str(i) + ": pre: " + " ".join(pred).encode("utf-8", "ignore") + "\n"
                )
                f.write(
                    str(i) + ": tgt: " + " ".join(tgt).encode("utf-8", "ignore") + "\n"
                )
