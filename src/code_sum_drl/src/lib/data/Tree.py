...
def traverse_python_tree(atok, root):
    ...
    for node in asttokens.util.walk(root):
        ...
        if not next(iter_children(node), None) is None:
            ...
            for child in iter_children(node):
                child_idx += 1
                node_json["%s%s" % (Constants.NODE_FIX, current_idx)]["children"].insert(
                    0,
                    "%s%s"
                    % (Constants.NODE_FIX, current_global[current_idx] - child_idx + 1),
                )
        else:  # leaf node
            node_json["%s%s" % (Constants.NODE_FIX, current_idx)]["children"].append(
                atok.get_text(node)
            )
...
def traverse_java_tree(tree, node_json, idx=1, global_idx=1):
    ...
    if tree.getChildCount() == 0:
        ...
        for c in tree.getChildren():
            ...
            global_idx += 1
            node_json["%s%s" % (Constants.NODE_FIX, current_idx)]["children"].append(
                "%s%s" % (Constants.NODE_FIX, global_idx)
            )
        for c in tree.getChildren():
            cx += 1
...
def split_tree(tree_json, idx_upper):
    ...
    for k, node in tree_json.items():
        if len(node["children"]) > 2:
            tree_json_splited["%s%s" % (Constants.NODE_FIX, idx_upper + 1)] = {
                "node": "Tmp",
            ...
            tree_json_splited[k]["children"] = [
                tree_json_splited[k]["children"][0],
                "%s%s" % (Constants.NODE_FIX, idx_upper + 1),
            ]  # idx_upper + 2

            idx_upper += 1
    for k, node in tree_json_splited.items():
        children_length = len(
            [c for c in node["children"] if c.startswith(Constants.NODE_FIX)]
        )
