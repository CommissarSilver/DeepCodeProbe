...
class ReinforceTrainer(object):
    ...
    def train(self, start_epoch, end_epoch, pretrain_critic, start_time=None):
        ...
        for epoch in range(start_epoch, end_epoch + 1):
            ...
            print(
                "Actor optim lr: %g; Critic optim lr: %g"
                % (self.optim.lr, self.critic_optim.lr)
            )
            if pretrain_critic:
                print("Pretrain critic...")
            no_update = (
                self.opt.no_update and (not pretrain_critic) and (epoch == start_epoch)
            )
            ...
            train_reward, critic_loss = self.train_epoch(
                epoch, pretrain_critic, no_update
            )
            ...
            model_name = os.path.join(
                self.opt.save_dir,
                "model_rf_%s_%s_%s" % (self.opt.data_type, self.opt.has_attn, epoch),
            )
            if pretrain_critic:
        ...
        for i in range(len(self.train_data)):  #
            ...
            if self.opt.has_attn:
                if self.opt.data_type == "code" or self.opt.data_type == "text":
                    ...
                    self.actor.decoder.attn.applyMask(
                        attention_mask_code, attention_mask_txt
                    )
            ...
            if not no_update:
                if self.opt.data_type == "code":
                    baselines = self.critic(
                        (batch[0], batch[1], samples, batch[3]),
                        eval=False,
                        regression=True,
                    )
                elif self.opt.data_type == "text":
            ...
            print(
                "iteration: %s, reward: %s "
                % (self.opt.iteration, (report_reward / report_sents) * 100)
            )

            if i % self.opt.log_interval == 0 and i > 0:
                print(
                    """Epoch %3d, %6d/%d batches; actor reward: %.4f; critic loss: %f; %5.0f tokens/s; %s elapsed"""
                    % (
                        epoch,
                        i,
                        len(self.train_data),
