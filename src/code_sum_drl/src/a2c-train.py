...
def get_opt():
    ...
    parser.add_argument(
        "-eval_one", action="store_true", help="Evaluate only one sample."
    )
    parser.add_argument(
    ...
    # Reward shaping
    parser.add_argument(
        "-pert_func", type=str, default=None, help="Reward-shaping function."
    )
    parser.add_argument(
        "-pert_param", type=float, default=None, help="Reward-shaping parameter."
    )
...
def load_data(opt):
    ...
    supervised_data = lib.Dataset(
        dataset["train_xe"], opt.batch_size, opt.cuda, eval=False
    )
    ...
    vis_data = lib.Dataset(
        dataset["test"], 1, opt.cuda, eval=True
...
def main():
    ...
    if use_critic:
        assert (
            opt.start_epoch + opt.critic_pretrain_epochs - 1 <= opt.end_epoch
        ), "Please increase -end_epoch to perform pretraining!"
    ...
    if opt.eval:
        ...
        reinforce_trainer = lib.ReinforceTrainer(
            model, critic, rl_data, test_data, metrics, dicts, optim, critic_optim, opt
        )
        ...
        xent_trainer = lib.Trainer(
            model, supervised_data, valid_data, metrics, dicts, optim, opt
        )
        if use_critic:
