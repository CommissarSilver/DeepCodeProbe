    ...
    def ast2binary_aux(current_node_G, G, new_G, parent_in_new_G):
        ...
        if len(out_edges) == 2:
            ...
            if not G.nodes[m]["is_terminal"]:
                new_G.nodes[parent_in_new_G]["type"] = (
                    new_G.nodes[parent_in_new_G]["type"] + SEPARATOR + G.nodes[m]["type"]
                )
                ast2binary_aux(m, G, new_G, parent_in_new_G)
            else:
                # todo: check this, unary things
                new_G.nodes[parent_in_new_G]["is_terminal"] = True
                new_G.nodes[parent_in_new_G]["unary"] = new_G.nodes[parent_in_new_G][
                    "type"
                ]
            ...
            for j, out_node in enumerate(out_nodes[1:]):
                if (
                    len(list(new_G.out_edges(new_empty_id))) == 1
                    and len(out_nodes[1:]) - j > 1
                ):
...
def distance_to_tree(d, c, u, tokens):
    def distance_to_tree_aux(G, d, c, u, father, tokens, start_token):
        if d == []:
            ...
            if father != None:
                G.add_edge(father, new_id)
            distance_to_tree_aux(
                G, d[0:i], c[0:i], u[0 : i + 1], new_id, tokens[0 : i + 1], start_token
            )
            distance_to_tree_aux(
                G,
                d[i + 1 :],
...
def extend_complex_nodes(G):
    g = G.copy()
    while (
        len([n for n in g if SEPARATOR in g.nodes[n]["type"] and g.out_edges(n) != 0])
        != 0
    ):
        g0 = g.copy()
        ...
        if G.out_degree(n) > 0:
            if filter_non_terminal is None:
                ...
                result.append(
                    G.nodes[n]["type"]
                    + "-"
                    + "-".join([str(G.nodes[l]["start"]) for l in leaves])
                )
            else:
