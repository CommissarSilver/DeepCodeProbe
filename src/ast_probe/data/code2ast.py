...
def solve_string_problems(G):
    strings = [
        n
        for n in G
        if (G.nodes[n]["type"] == "string" or ("string_literal" in G.nodes[n]["type"]))
...
def code2ast(code, parser, lang="python"):
    if lang == "python":
        ...
        get_graph_from_tree(tree.root_node, G, 0)
    # @TODO: #1 add support for C since we use it in AstNN
...
def remove_comments_ast(G, code):
    start_end_comments = [
        (G.nodes[n]["start"], G.nodes[n]["end"])
        for n in G
        if G.nodes[n]["type"] == "comment"
    ]
...
def get_tokens_ast(T, code):
    return [
        get_token(code, T.nodes[t]["start"], T.nodes[t]["end"])
        for t in sorted(
            [n for n in T if T.nodes[n]["is_terminal"]], key=lambda n: T.nodes[n]["start"]
        )
    ]
