...
class InferCodeClient(BaseClient):
    ...
    def __init__(self, language, load_model=True):
    ...
    def init_from_config(self, config=None):
        ...
        self.infercode_model = InferCodeModel(
            num_types=self.node_type_vocab.get_vocabulary_size(),
    ...
    def snippets_to_tensors(self, batch_code_snippets):
        ...
        for code_snippet in batch_code_snippets:
            ...
            tree_representation, _ = self.ast_util.simplify_ast(
                ast, code_snippet_to_byte
            )
    ...
    def encode(self, batch_code_snippets):
        ...
        embeddings = self.sess.run(
            ...
            feed_dict={
                self.infercode_model.placeholders["node_type"]: tensors[
                    "batch_node_type_id"
                ],
                self.infercode_model.placeholders["node_tokens"]: tensors[
                    "batch_node_tokens_id"
                ],
                self.infercode_model.placeholders["children_index"]: tensors[
                    "batch_children_index"
                ],
                self.infercode_model.placeholders["children_node_type"]: tensors[
                    "batch_children_node_type_id"
                ],
                self.infercode_model.placeholders["children_node_tokens"]: tensors[
                    "batch_children_node_tokens_id"
                ],
                self.infercode_model.placeholders[
                    "language_index"
                ]: self.language_index,
