...
class funcGNN(torch.nn.Module):
    ...
    def setup_layers(self):
        ...
        self.convolution_1 = SAGEConv(
            self.number_labels, self.args.filters_1, normalize=True
        )
        self.convolution_2 = SAGEConv(
            self.args.filters_1, self.args.filters_2, normalize=True
        )
        self.convolution_3 = SAGEConv(
            self.args.filters_2, self.args.filters_3, normalize=True
        )
...
class funcGNNTrainer(object):
    ...
    def get_train_baseline_error(self):
        ...
        base_train_error = np.mean(
            [(n - norm_ged_mean) ** 2 for n in self.train_ground_truth]
        )
    ...
    def fit(self):
        ...
        for epoch in epochs:
            ...
            if not os.path.exists("./outputFiles/test"):
                os.makedirs("./outputFiles/test")
            with open(
                "./outputFiles/test/train_error_graph.txt", "a"
            ) as train_error_writer:
                train_error_writer.write(
                    str(epoch_counter) + "," + str(round(loss, 6)) + "\n"
                )
    ...
    def runParallelCode(self, pairList):
        ...
        with cf.ProcessPoolExecutor(max_workers=5) as executor:
            results = [
                executor.submit(self.load_model_parallel, files) for files in pairList
            ]
